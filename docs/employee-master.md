...
> **状態: v1 実装済み（2026-07-13）**。最小 3 列 `employee_id` / `office_id` / `role(primary|support)` で
> 事業所またぎ（応援）勤務を実装した（マイグレーション `20260713000200_add_employee_office_assignment`、
> 既存 `Employee.office_id` から `role=primary` をバックフィル）。`allowed_pattern_codes` / `time_scope` /
> `condition_note` / `target_work_days` / `priority` は将来拡張（下表は完成形の設計）。
>
> - `role=support`（応援先）を持つ職員は、その事業所の勤務表に行が出て、自動生成でも配置対象に含まれる。
> - 勤務表の編集は主たる所属（primary）拠点でのみ。各セルは勤務先事業所を持ち、共通記号は
>   セルの事業所トグルで主/応援先を切り替える（[screen-list.md](screen-list.md) S-A-08）。
> - `Employee.office_id` は primary の唯一の真実として温存（v1 の primary 判定は office_id 一本）。
> - **梨花の氏名ベース `isHelper`（`src/lib/shift/rika/config.ts`）は当面据え置き**。将来この
>   assignment へ統一する（§8.2）。夜勤上限・連勤の拠点横断集計は v1 未対応（手動チェック）。
...
>
> **夜勤希望回数**（月◯回入りたいか）は上限とは別軸のため `Employee.desired_night_shifts_per_month`
> に持つ（自動作成 v2 Phase 2 で優先配置に使用、[auto-shift-design-v2.md §4.2 ①](auto-shift-design-v2.md)）。
> `max_night_shifts_per_month` が「上限」、こちらが「希望」（希望 ≤ 上限が通常）。
> CSV に専用列が無い場合は取り込み後に従業員編集 UI で手入力する。
...
| 夜勤希望回数（あれば）                  | `Employee.desired_night_shifts_per_month`                         | **新列**（無ければ UI 手入力）         |
