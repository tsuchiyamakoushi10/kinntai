...
- 日本の祝日判定は `src/lib/calendar/holidays.ts` に切り出し、内閣府公表データを年度ごとに更新する（MVP はハードコード、Future でテーブル化を検討）
- `required_count` は 0 以上の整数。空欄 = 0 として扱う（`null` は使わない）
...
- **営業日の判定**: `am + pm + night_in + night_out > 0` の日種を営業日とする（専用フラグは持たない。v2 §8④と整合）。例: デイの `sunday_holiday` を全 0 にすれば日曜休業
...
- **`office_shift_quotas`（[§2.11](#211-office_shift_quotas--拠点別-シフト必要人員数)、パターン単位）との関係**: v1/v2 初期はパターン単位、案A はこの午前/午後単位。どちらを生成の需要に使うかは生成ロジック側（拠点別）で決める。当面 両テーブルは併存し、案A 対象拠点は本テーブルを使う
