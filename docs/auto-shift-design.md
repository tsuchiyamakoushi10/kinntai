...
- 拠点をまたぐ応援勤務の**最適化**（拠点横断の夜勤上限・連勤・勤務日数合算）。
  - v1（2026-07-13）で、応援先（`employee_office_assignment.role=support`）職員を各拠点の自動作成の
    配置対象に含め、他拠点で既に埋まっている日を勤務不可として避けるところまで対応した
    （`src/lib/shift/cross-office.ts`）。ただし**実行順依存**（先に生成した拠点が優先）で、
    夜勤上限・連勤・勤務日数の拠点横断集計は未対応（手動チェック）。運用は「主拠点→応援先」順で回す。
- 複数月一括の最適化。
